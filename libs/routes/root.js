...
router.get('/download/js', (req, res) => {
...
router.get('/apps', auth.presets, (req, res) => {
    actions.getAllKeys(req, res, (err, keys) => {
        res.render('keys', { user: req.user, title: 'All Keys - Trackful', keys, heading: 'Your applications', subtype: 'Applications' });
    });
});
...
router.get('/app/:key/:type*?', auth.presets, (req, res) => {
    actions.validateKeyOwner(req, res, req.params.key, (err, owner) => {
        if (!err && owner && ['activity', 'clicks', 'hits', 'countries', 'devices', 'sessions', undefined].includes(req.params.type)) {
            if (req.params.type === undefined) { req.params.type = 'dashboard'; }

            async.parallel({
                trackers: (callback) => {
                    actions.getTrackers(req, res, req.params.key, (e, clicksTrackers, hitsTrackers, countriesTrackers, devicesTrackers, sessionTrackers, sessionTime, hasClickTrackers) => {
                        if (!e) { callback(null, clicksTrackers, hitsTrackers, countriesTrackers, devicesTrackers, sessionTrackers, sessionTime, hasClickTrackers); }
                    });
                },
                key: (callback) => {
                    actions.getKeyInfo(req, res, req.params.key, (e, info) => {
                        if (!e) callback(null, info);
                    });
                },
                activity: (callback) => {
                    if (req.params.type === 'activity') {
                        actions.getActivity(req, res, req.params.key, (e, activities) => {
                            if (!e) callback(null, activities);
                        });
                    } else {
                        callback(null, [false]);
                    }
